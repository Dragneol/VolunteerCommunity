    ...
    public class AccountController : ApiController
    {
        ...
        public List<Statistics> Statistic()
        {
            ...
            for (int i = 1; i <= curentime.Month; i++)
            {
                ...
                numberOfAccount = UserManager.Users.Where(p => p.TimeCreated >= from && p.TimeCreated < to).Count();
                statistics.Add(new Statistics() { month = i, value = numberOfAccount });
            }
            return statistics;
        }
        //Statistics number of evnt had joined and number of org of a user by userID
        [Route("StatisticByUserID")]
        [HttpGet]
        public UserStatistics StatisticByUserID(string UserID)
        {
            UserStatistics statistics = new UserStatistics();
            statistics.Event = db.EventVolunteers.Where(p => p.AccountID.Equals(UserID)).Count();
            statistics.Organization = db.OrganizationMembers.Where(p => p.AccountID.Equals(UserID)).Count();
            return statistics;
        }

        // GET: Count how many users in the system
        [Route("CountUsers")]
        ...
        // GET: Get all accounts with specific role
        [Route("AccountsInRole")]
        public IEnumerable<Account> GetAccountsInRole(string role)
        {
            return UserManager.Users.ToList().Where(user => UserManager.IsInRole(user.Id, role));
        }
        ...
        public IHttpActionResult PutAccount(Account @account)
        {
            ...
            if (User.Identity.GetUserId() != @account.Id)
            {
                return BadRequest();
            }
        ...
        [Route("Disable")]
        [HttpPost]
        public IHttpActionResult DisableUser(string userId)
        {
            UserManager.SetLockoutEnabled(userId, true);
            db.SaveChanges();
            return Ok("Disable success");
        }
        // POST: API Enable account
        [Route("Enable")]
        [HttpPost]
        public IHttpActionResult EnableUser(string userId)
        {
            UserManager.SetLockoutEnabled(userId, false);
            db.SaveChanges();
            return Ok("Enable success");
        }
        ...
        public async Task<IHttpActionResult> GetExternalLogin(string provider, string error = null)
        {
            ...
            if (hasRegistered)
            {
                ...
                AuthenticationProperties properties = ApplicationOAuthProvider.CreateProperties(user, false);
